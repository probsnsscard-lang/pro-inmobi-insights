...
export function generatePDF(
  ...
  // ── Block 1: VALOR COMERCIAL ESTIMADO (PRIMERO) ──
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...navyDark);
  doc.text('■ VALOR COMERCIAL ESTIMADO (Basado en Corazón de Mercado)', marginL, y);
  y += 6;
  ...
  if (isTerrain) {
    ...
    doc.setTextColor(...white);
    let bx = marginL + 2;
    breakdownHeader.forEach(h => { doc.text(h.text, bx, y + 5); bx += h.width; });
    y += 7;
  ...
  // ── Block 2: Características de la Propiedad ──
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...navyDark);
  ...
  // (Valor comercial ya fue renderizado arriba como Block 1)

  // ── Block 3: Estadísticas del Mercado ──
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...navyDark);
